...
public class NativeBridge
{
    ...
    private static string _libName = String.Empty;
    ...
    public bool Initialize()
    {
        try
        {
            _libName = RuntimeInformation.IsOSPlatform(OSPlatform.Windows)
                ? "counterstrikesharp.dll"
                : "counterstrikesharp.so";

            _libHandle = NativeLibrary.Load(_libName);
            _invokeNative = GetDelegate<InvokeNativeDelegate>("InvokeNative");

            _logger.LogInformation("Loaded native lib OK ({Library})", _libName);
            return true;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to load native lib '{Library}'", _libName);
            return false;
        }
    }
